...
public class ExcelUtil {
     ...
     * @param result    // 表数据
     */
    public static void createTable(HSSFWorkbook wb,HSSFSheet sheet,List<Long> result){
        ...
        for(int i = 0; i < result.size()/7; i++ ){
            ...
            for (int j = 0; j < 7; j++) {
                ...
                cell.setCellStyle(cellStyle);
                if(j == 0) {
                	cell.setCellValue(result.get(i * 7 + j).toString() + "个");
                }else {
                	cell.setCellValue(result.get(i * 7 + j));
                }
            }
        }
    }
